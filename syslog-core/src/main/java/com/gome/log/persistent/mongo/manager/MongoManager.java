package com.gome.log.persistent.mongo.manager;

import com.gome.log.persistent.mongo.config.MongoDBConfig;
...
public class MongoManager {
    ...
    static{
        try {
            if (mongoDBConfig==null){
                logger.error("mongo not config,please check mongo db config!");
                throw new Exception("mongo not config,please check mongo db config!");
            }
            ...
            mongo = new Mongo(new ServerAddress(mongoDBConfig.getHost(),mongoDBConfig.getPort()),options);
    ...
    public static void setMongoDBConfig(MongoDBConfig mongoDBConfig) {
        MongoManager.mongoDBConfig = mongoDBConfig;
    }
}
